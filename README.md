...
- ESP32 v5 przechowuje surowe wiersze NMEA w pamięci flash, kompresuje je strumieniowo zlib/DEFLATE w `loop()` i udostępnia przez BLE NOTIFY. W trybie testowym wysyła tę trasę 10 razy, używa najszybszego potwierdzonego profilu ze statycznym pacingiem NOTIFY 4 ms. Protokół wire pozostaje kompatybilny z v4.
- PWA v5 dekompresuje dane do identycznego CSV, zapisuje transfer strumieniowo do OPFS, gdy przeglądarka je udostępnia, i raportuje profil czasowy całego transferu. Przed podsumowaniem czeka również na zakończenie wszystkich zapisów ACK/NACK.
...
4. Naciśnij **Pobierz NMEA CSV**. PWA v5 włączy `NOTIFY`, wyśle `START`, a ESP32 v5 rozpocznie dziesięciokrotną kompresję w `loop()`.
5. Po zakończeniu PWA v5 pokaże czas, liczbę pakietów, bitrate skompresowany, średni bitrate CSV, redukcję, CRC32, profil PWA i próbkę odebranego CSV. Dla dokładnego benchmarku pozostaw wyłączony szczegółowy log ramek.
...
- `notify` - czas wywołań `setValue()` i `notify()`.
