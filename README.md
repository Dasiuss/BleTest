...
- ESP32 v4.6 przechowuje surowe wiersze NMEA w pamięci flash, kompresuje je strumieniowo zlib/DEFLATE w `loop()` i udostępnia przez BLE NOTIFY. W trybie testowym wysyła tę trasę 10 razy, zaczyna od tempa NOTIFY 4 ms i automatycznie zwalnia po błędzie alokacji NimBLE. Protokół pozostaje kompatybilny z v4.
- PWA v4.4 dekompresuje dane do identycznego CSV, zapisuje transfer strumieniowo do OPFS, gdy przeglądarka je udostępnia, i raportuje profil czasowy całego transferu. Przed podsumowaniem czeka również na zakończenie wszystkich zapisów ACK/NACK.
...
7. Otwórz Serial Monitor. Powinien pojawić się komunikat `GPS route test v4.6` oraz `Advertising: BleTestEsp32`.
...
4. Naciśnij **Pobierz NMEA CSV**. PWA v4.7 włączy `NOTIFY`, wyśle `START`, a ESP32 v4.6 rozpocznie dziesięciokrotną kompresję w `loop()`.
5. Po zakończeniu PWA v4.4 pokaże czas, liczbę pakietów, bitrate skompresowany, średni bitrate CSV, redukcję, CRC32, profil PWA i próbkę odebranego CSV. Dla dokładnego benchmarku pozostaw wyłączony szczegółowy log ramek.
...
Wymiana MTU jest negocjacją obu stron. `BLEDevice::setMTU(517)` ustawia maksimum po stronie ESP32, ale klient GATT musi zainicjować wymianę. Ramka NOTIFY ma 4 bajty numeru sekwencyjnego i do 240 bajtów skompresowanego strumienia. ESP32 utrzymuje maksymalnie 64 ramki w locie, a PWA wysyła skumulowany `ACK:<next_sequence>` co 32 ramki, zwalniając kolejne sloty bez zatrzymywania kompresora. Bufor ramek zajmuje około 16 KiB i nie ogranicza rozmiaru całego pliku, który jest przesyłany strumieniowo. Przy luce PWA wysyła `NACK:<expected_sequence>`, a ESP32 powtarza niepotwierdzone ramki z bufora. Pusta ramka kończy transfer. Po transferze PWA weryfikuje rozmiar, liczbę linii i CRC32 odtworzonego pliku.
