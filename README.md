...
- ESP32 v4.4 przechowuje surowe wiersze NMEA w pamięci flash, kompresuje je strumieniowo zlib/DEFLATE w `loop()` i udostępnia przez BLE NOTIFY. W trybie testowym wysyła tę trasę 10 razy i ogranicza tempo NOTIFY do 8 ms, aby nie przepełnić kolejki NimBLE. Protokół pozostaje kompatybilny z v4.
...
7. Otwórz Serial Monitor. Powinien pojawić się komunikat `GPS route test v4.4` oraz `Advertising: BleTestEsp32`.
...
4. Naciśnij **Pobierz NMEA CSV**. PWA v4.5 włączy `NOTIFY`, wyśle `START`, a ESP32 v4.4 rozpocznie dziesięciokrotną kompresję w `loop()`.
